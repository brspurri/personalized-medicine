...
def print_help():
    print "clean_pdb.py <pdb> <chain id> <output_pdb>"
    ...
    print "\n",
    print "output_pdb = path to write use for the cleaned pdb."
...
for i in range(len(lines)):
    ...
    if len(line) > 20 and line[21] in chainid or ignorechain or removechain:
        line_edit = line
        if line[0:3] == 'TER':
            continue
        elif line[0:3] == 'END':
...
if nres > 0:
    ...
    if argv[2] != "ignorechain" and argv[2] != "nochain":
        for chain in fastaseq:
            pdbname = os.path.basename(pdbname)
            fastaid.write('>'+basename(pdbname)[0:4]+"_template" + '\n')
            fastaid.write(fastaseq[chain])
            fastaid.write('\n')

            # handle = open(pdbname[0:4]+"_template" + ".fasta", 'w')
            # handle.write('>'+pdbname[0:4]+"_template"+'\n')
